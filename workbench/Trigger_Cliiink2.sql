...
BEGIN
SELECT COUNT(en_id) AS nDoublons, en_nom, en_siren, en_nic, en_adresse, en_code_postal, en_vi_id_fk, en_sa_id_fk FROM entreprise
GROUP BY en_nom, en_siren, en_nic, en_adresse, en_code_postal, en_vi_id_fk, en_sa_id_fk
HAVING COUNT(en_id) > 1;
...
BEGIN
	IF EXISTS (SELECT COUNT(en_id) AS nDoublons, en_nom, en_siren, en_nic, en_adresse, en_code_postal, en_vi_id_fk, en_sa_id_fk 
			   FROM entreprise
			   GROUP BY en_nom, en_siren, en_nic, en_adresse, en_code_postal, en_vi_id_fk, en_sa_id_fk
			   HAVING COUNT(en_id) > 1) THEN 
...
-- ------------------------------
-- PS DOUBLONS secteur_activite
-- ------------------------------
# Procédure stockée pour vérifier les doublons sur la table secteur_activite
...
BEGIN
SELECT DISTINCT *
FROM secteur_activite AS sa1
WHERE EXISTS (
              SELECT *
              FROM secteur_activite AS sa2
...
-- ----------------------------
-- TRIGGER doublons secteur_activite
-- ----------------------------
...
CREATE TRIGGER td_sa
    AFTER UPDATE ON secteur_activite
    FOR EACH ROW 

BEGIN
	IF EXISTS (SELECT DISTINCT * FROM secteur_activite AS sa1 WHERE EXISTS (SELECT * 
    FROM secteur_activite AS sa2 WHERE sa1.sa_id <> sa2.sa_id 
